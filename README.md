...
Lipo-Nanny lets RC pilots focus on flying. The EdgeTX script watches the flight battery over ELRS telemetry and speaks up on its own — twice per flight, when it's time to return and when it's time to land.
...
- During flight, the FC-reported mAh counter (CRSF `Capa`) is offset by the start SoC, so remaining capacity reflects reality from the first second.
...
- Each pack can carry a **wear %** that lowers its effective capacity, so an aging battery triggers the warnings **earlier** — no need to re-do your thresholds as a battery gets tired.
...
2. **(Optional) Add the voice files** so the warnings are spoken — supply your own WAV files at:
...
| `Sensor missing` | A required sensor (`RxBt` or `Capa`) isn't present. Run **Discover sensors** in EdgeTX telemetry. |
...
**No voice warning?** Check that `warn.wav` / `crit.wav` exist and the radio volume is up. Warnings require the `Capa` (consumed-mAh) sensor — there is deliberately no voltage-only fallback.

**Widget blank or not updating?** It must sit on a telemetry screen; without placement EdgeTX never runs it.
